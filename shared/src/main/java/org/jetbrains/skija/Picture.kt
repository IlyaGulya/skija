...
class Picture internal constructor(
    ...
    companion object {
        ...
        internal external fun _nPlayback(ptr: Long, canvasPtr: Long, abort: BooleanSupplier?)
        internal external fun _nGetCullRect(ptr: Long): Rect
        internal external fun _nGetUniqueId(ptr: Long): Int
        internal external fun _nSerializeToData(ptr: Long /*, SkSerialProcs */): Long
        internal external fun _nMakePlaceholder(left: Float, top: Float, right: Float, bottom: Float): Long
        internal external fun _nGetApproximateOpCount(ptr: Long): Int
        internal external fun _nGetApproximateBytesUsed(ptr: Long): Long
        internal external fun _nMakeShader(
            ptr: Long,
            tmx: Int,
            tmy: Int,
            filterMode: Int,
    ...
    fun playback(canvas: Canvas?, abort: BooleanSupplier? = null): Picture {
        return try {
            Stats.onNativeCall()
            _nPlayback(ptr, getPtr(canvas), abort)
            this
        } finally {
    ...
    fun makeShader(
        ...
        return try {
            ...
            Shader(_nMakeShader(ptr, tmx.ordinal, tmy.ordinal, mode.ordinal, arr, tileRect))
        } finally {
            Reference.reachabilityFence(this)
        }
    }
